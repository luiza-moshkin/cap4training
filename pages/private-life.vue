<template>
  <h3 style="text-align: center;"> {{ $t('private-life') }}</h3>
...
var { data: generalCondition } = await reactive(await useAsyncData(`privatelife-${locale.value}/`, () =>
  queryContent(`/privatelife/${locale.value}/private-life-2`).findOne())
);
